//code made by Rhian Feipe, the script aims to encrypt and decrypt the cesar cipher
...
int main(void)
{
    ...
    fgets(phrase,100,stdin); //picking up the phrase
    
    
    printf("Enter the desired Keyn size\n");
    scanf(" %i",&key);//picking up the key
    printf("Type C to Encrypt, or D to Decrypt the entered word/phrase\n");
    scanf(" %c", &C_or_D); // whether to encrypt or decrypt


    if (C_or_D == 'C' || C_or_D == 'c') {// if encrypt or decrypt


       cryptographyAndDecryption(phrase, key); //calling function
